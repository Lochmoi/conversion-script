...
# =========================================================================
# .mkv /mp3 media processing with INTELLIGENT SIZING
# =========================================================================
# Features:
...
# CONFIGURATION
# =========================================================================
SOURCE_DIR="/home/zambia/renamed"              # Directory containing source files
UPLOAD_DIR="/home/backup-6"                    # Directory for processed chunks
MOTHER_DIR="/home/zambia/mother"               # Archive directory for originals
...
VIDEO_CODEC="libx264"
VIDEO_PRESET="ultrafast"                   # ultrafast = original speed (change to "medium" for better quality)
VIDEO_SCALE="-2:480"                       # Scale to 480p height (preserve quality)
...
#   - Stream copy, no re-encoding (like your simple convert_mbt.sh)
...
# Get duration of media file in seconds (FIXED - handles corrupt files)
get_duration_seconds() {
    ...
    if [ -z "$duration_hms" ] || [[ "$duration_hms" == *"N/A"* ]]; then
        echo_error "Cannot determine duration for: $(basename "$file")"
        echo "0"
        return
    fi
    ...
    h=$(echo "$h" | sed 's/^0*\([0-9]\)/\1/')
    m=$(echo "$m" | sed 's/^0*\([0-9]\)/\1/')
    s=$(echo "$s" | sed 's/^0*\([0-9]\)/\1/')
    ...
    if ! [[ "$h" =~ ^[0-9]+$ ]]; then h=0; fi
    if ! [[ "$m" =~ ^[0-9]+$ ]]; then m=0; fi
    if ! [[ "$s" =~ ^[0-9]+$ ]]; then s=0; fi
    
    # Calculate total seconds
    local total_seconds=$(( (h * 60 + m) * 60 + s ))
    echo "$total_seconds"
}
...
# Process MP3 file - split into 10-minute chunks (NO CHANGES - already optimal)
process_mp3_file() {
...
process_mkv_file() {
    ...
    if [ "$USE_COPY_MODE" = true ]; then
        # COPY MODE: Stream copy (no re-encoding) - ULTRA FAST like your simple script
        ...
        # FAST MODE: Single-pass CBR for speed (original speed)
        ...
        if [ -z "$source_duration" ] || [ "$source_duration" -le 0 ]; then
            echo_error "[STEP 1] Cannot determine valid duration. File may be corrupt."
            rm -rf "$work_dir"
            return 1
        fi
        
        echo_info "[STEP 1] Source duration: ${source_duration} seconds"
        ...
        if [ "$FFMPEG_RC" -eq 0 ]; then
            local output_size_mb=$(stat -c%s "$mp4_file" 2>/dev/null)
            output_size_mb=$((output_size_mb / 1048576))
            echo_success "[STEP 1] Conversion complete: Output ${output_size_mb}MB"
        else
        ...
        # Safety check: prevent division by zero (CRITICAL FIX)
        if [ -z "$source_duration" ] || [ "$source_duration" -le 0 ]; then
            echo_error "[STEP 1] Cannot determine valid source duration (got: ${source_duration:-empty})"
            echo_error "[STEP 1] File may be corrupt or too short"
            rm -rf "$work_dir"
            return 1
        fi
    ...
    if [ "$duration" -le 0 ]; then
        echo_error "[STEP 2] Invalid duration for converted file"
        rm -rf "$work_dir"
        return 1
    fi
    ...
    while [ $offset -lt $duration ]; do
        ...
        if ffmpeg -i "$mp4_file" -vcodec copy -acodec copy \
            ...
            if mv "$chunk_file" "$UPLOAD_DIR/$new_name" 2>/dev/null; then
                local chunk_size_mb=$(stat -c%s "$UPLOAD_DIR/$new_name" 2>/dev/null)
                chunk_size_mb=$((chunk_size_mb / 1048576))
            ...
            echo_error "[STEP 2] Failed to create chunk $chunk_num"
        fi
...
# Register a background job (FIXED - creates PID dir if needed)
register_job() {
    local pid=$1
    local filename=$2
    
    # Ensure PID directory exists (critical for subprocesses)
    if [ ! -d "$PID_DIR" ]; then
...
# Count ACTUAL active jobs (only this script's jobs)
count_active_jobs() {
    local count=0
    shopt -s nullglob
...
process_files_concurrently() {
    ...
    while [ $mkv_idx -lt $total_mkv ] || [ $mp3_idx -lt $total_mp3 ]; do
        ...
        # Start next MKV job if available and slot is free
        ...
        # Start next MP3 job if available and slot is free
...
main() {
    ...
    if [ "$USE_COPY_MODE" = true ]; then
        ...
        echo_info "Mode: FAST (single-pass CBR)"
