...
# =========================================================================
#.mkv to mp4 media conversion with intelligent file size
# =========================================================================
# Features:
# - Converts .mkv files to .mp4 and splits into 10-minute chunks
# - MP3 PROCESSING: commented out
...
# - Concurrent file processing with robust job tracking
...
MAX_PARALLEL_JOBS=4                            # Maximum concurrent processing jobs (FIXED: safe for 12GB RAM)
...
# SPEED CONTROL: Choose your mode
...
# Process MP3 file - split into 10-minute chunks with ROBUST error handling
# COMMENTED OUT - MP3 processing disabled
process_mp3_file() {
    return 0  # Skip MP3 processing
}

# MP3 PROCESSING COMMENTED OUT for future use
# process_mp3_file() {
...
#     if [ ! -f "$mp3_filepath" ]; then
#         echo_error "File does not exist: $mp3_filepath"
#         return 1
#     fi
#     
#     local filesize=$(stat -c%s "$mp3_filepath" 2>/dev/null || echo "0")
...
#     
#     # Create unique temporary directory (FIXED: using /var/tmp for disk, not RAM)
#     local work_dir="/var/tmp/mp3_$$_$(date +%s)_${RANDOM}"
#     mkdir -p "$work_dir"
#     
#     # Validate media file
...
#     fi
#     
#     # Get duration with robust fallback
#     local duration=$(get_duration_seconds "$mp3_filepath")
#     if [ "$duration" -le 0 ]; then
#         echo_warning "[STEP 1] Cannot determine exact duration, will attempt chunking anyway"
#         local mp3_size=$(stat -c%s "$mp3_filepath" 2>/dev/null)
#         local estimated_chunks=$(( (mp3_size / (5 * 1048576)) + 1 ))  # Estimate based on ~5MB per 10min
#         echo_info "[STEP 1] Will attempt to create approximately $estimated_chunks chunks"
...
#             
#             # CRITICAL: Validate chunk was created and has meaningful content
...
#                         echo_success "Chunk ready: $new_name (${chunk_size_mb}MB)"
#                         ((chunks_created++))
#                     else
...
#                     rm -f "$chunk_file"
#                     echo_info "Reached end of file (chunk too small)"
#                     break
#                 fi
#             else
#                 # No more chunks to create
#                 echo_info "No more chunks to create"
#                 break
#             fi
#         else
#             # Failed to create chunk
#             if [ $chunks_created -eq 0 ]; then
#                 echo_error "Failed to create any chunks"
#             else
#                 echo_info "Reached end of file after $chunks_created chunks"
#             fi
#             break
#         fi
#         
...
#             echo_warning "Stopping after 100 chunks (safety limit)"
#             break
#         fi
#     done
#     
...
#     rm -rf "$work_dir"
#     
#     # Archive original (only if we created chunks successfully)
...
#         return 0
#     else
#         echo_error "MP3 processing failed: $filename (no chunks created)"
#         return 1
#     fi
# }
...
process_mkv_file() {
    ...
    # Create unique temporary directory (FIXED: using /var/tmp for disk, not RAM)
    local work_dir="/var/tmp/mkv_$$_$(date +%s)_${RANDOM}"
    mkdir -p "$work_dir"
...
show_status() {
    ...
    if [ -d "$SOURCE_DIR" ]; then
        ...
        # local mp3_count=$(find "$SOURCE_DIR" -name "*.mp3" -type f 2>/dev/null | wc -l)  # MP3 DISABLED
        
        echo_info "Source Directory: $SOURCE_DIR"
        echo_info "  MKV files: $mkv_count"
        # echo_info "  MP3 files: $mp3_count"  # MP3 DISABLED
        ...
        if [ -n "$patterns" ]; then
            ...
            for pattern in $patterns; do
                ...
                # local mp3_cnt=$(find "$SOURCE_DIR" -name "${pattern}_*.mp3" -type f 2>/dev/null | wc -l)  # MP3 DISABLED
                if [ $mkv_cnt -gt 0 ]; then
                    echo_info "  $pattern: MKV=$mkv_cnt"
                    # echo_info "  $pattern: MKV=$mkv_cnt, MP3=$mp3_cnt"  # MP3 DISABLED
                fi
            done
        fi
    fi
    ...
    if [ -d "$UPLOAD_DIR" ]; then
        ...
        # local upload_mp3=$(find "$UPLOAD_DIR" -name "*.mp3" -type f 2>/dev/null | wc -l)  # MP3 DISABLED
        ...
        # echo_info "  MP3 chunks: $upload_mp3"  # MP3 DISABLED
...
# Wrapper for MP3 processing with job tracking
process_mp3_with_tracking() {
    local file="$1"
    local my_pid=$$
...
process_files_concurrently() {
    ...
    # MP3 SCANNING DISABLED (COMMENTED OUT)
    # echo_info "Scanning for MP3 files..."
    # while IFS= read -r -d '' file; do
    #     mp3_files+=("$file")
    ...
    local total_files=$((total_mkv))  # MP3 disabled, only count MKV
    ...
    echo_info "Found $total_mkv MKV files (MP3 processing disabled)"
    ...
    # Main processing loop (MP3 disabled, only process MKV)
    while [ $mkv_idx -lt $total_mkv ]; do
        ...
        # MP3 PROCESSING DISABLED (COMMENTED OUT)
        # # Process MP3 files
        # if [ $mp3_idx -lt $total_mp3 ] && [ $(count_active_jobs) -lt $MAX_PARALLEL_JOBS ]; then
        #     local file="${mp3_files[$mp3_idx]}"
        #     local fname=$(basename "$file")
        #     echo_info "[MP3 $((mp3_idx + 1))/$total_mp3] Starting: $fname"
        #     
        #     (
        #         if process_mp3_with_tracking "$file"; then
        #             echo_success "[MP3 $((mp3_idx + 1))/$total_mp3] Completed: $fname"
        #         else
        #             echo_error "[MP3 $((mp3_idx + 1))/$total_mp3] Failed: $fname"
        #         fi
        #     ) &
        #     
        #     local job_pid=$!
...
cleanup() {
    ...
    # FIXED: Clean up /var/tmp instead of /tmp
    rm -rf /var/tmp/mkv_$$_* 2>/dev/null || true
    rm -rf /var/tmp/mp3_$$_* 2>/dev/null || true
    cleanup_pid_dir
    rm -f "$LOCKFILE"
...
main() {
    ...
    echo_info "  (MP3 Processing: DISABLED)"
    echo_info "=========================================="
    ...
    echo_info "Parallel jobs: $MAX_PARALLEL_JOBS (optimized for 12GB RAM)"
    echo_info "Temp directory: /var/tmp (disk-based, OOM-safe)"
    echo_info "=========================================="
    echo ""
